...
pub(crate) fn render_tab_bar(
    ...
    for (idx, (item, rect)) in items.iter().zip(hitboxes.tabs.iter()).enumerate() {
        ...
        let text = if rect.width <= 3 {
            format!("{:>width$}", idx + 1, width = usize::from(rect.width))
        } else {
        ...
        let _ = buf.set_stringn(rect.x, rect.y, &text, usize::from(rect.width), style);
    }
