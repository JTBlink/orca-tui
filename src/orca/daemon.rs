...
pub struct HelloResponse {
    ...
    #[serde(alias = "daemon_identity", alias = "daemonIdentity")]
...
impl DaemonEndpoint {
    ...
    ///
    /// Orca stores daemon artifacts under `<userData>/daemon/daemon-v<N>.sock`.
    /// The socket filename is versioned — we scan for the highest version present.
    #[must_use]
    pub fn discover() -> Option<Self> {
        let version_suffix = format!("v{PROTOCOL_VERSION}");
        let candidates = [
            // macOS: ~/Library/Application Support/Orca/
            dirs::data_dir().map(|d| d.join("Orca")),
            // Linux: ~/.config/Orca/ (Electron default)
            dirs::config_dir().map(|d| d.join("Orca")),
            // XDG: ~/.local/share/Orca/
            dirs::data_dir().map(|d| d.join("Orca")),
            // orcad: ~/.orca/
            dirs::home_dir().map(|d| d.join(".orca")),
            // Lowercase fallbacks
            dirs::config_dir().map(|d| d.join("orca")),
            dirs::data_dir().map(|d| d.join("orca")),
        ];
        for base in candidates.into_iter().flatten() {
            ...
            // Versioned paths (current Orca format)
            let socket = daemon_dir.join(format!("daemon-{version_suffix}.sock"));
            let token = daemon_dir.join(format!("daemon-{version_suffix}.token"));
            ...
            // Legacy unversioned paths (older Orca or custom setups)
...
mod tests {
    ...
    #[test]
    fn hello_response_ok_with_camel_case_identity_deserializes() {
